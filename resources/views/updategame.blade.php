    ...
    <form action="/UpdateDrink/{{$drinkDetail->id}}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="category" aria-selected="Action">
                <option value="Idle" {{$drinkDetail->category == "Idle"? "Selected":""}}>Idle</option>
                <option value="Horror" {{$drinkDetail->category == "Horror"? "Selected":""}}>Horror</option>
                <option value="Adventure" {{$drinkDetail->category == "Adventure"? "Selected":""}}>Adventure</option>
                <option value="Action" {{$drinkDetail->category == "Action"? "Selected":""}}>Action</option>
                <option value="Sports" {{$drinkDetail->category == "Sports"? "Selected":""}}>Sports</option>
                <option value="Strategy" {{$drinkDetail->category == "Strategy"? "Selected":""}}>Strategy</option>
                <option value="Role-playing" {{$drinkDetail->category == "Role-playing"? "Selected":""}}>Role-playing</option>
                <option value="Puzzle" {{$drinkDetail->category == "Puzzle"? "Selected":""}}>Puzzle</option>
                <option value="Simulation" {{$drinkDetail->category == "Simulation"? "Selected":""}}>Simulation</option>
            </select>
        </div>
        <div class="mb-3">
